...
//===== DANH MỤC NỔI BẬT ======
$model        = new Model();
$query        = "SELECT `name`,`id` FROM `" . TBL_CATEGORY . "` WHERE `status`= 'active' ORDER BY `ordering` LIMIT 0,4";
$arrCategory  = $model->fetchAll($query);
$xhtml = '';
foreach ($arrCategory as $value) {
    $id     = $value['id'];
    $name   = $value['name'];
    $action = 'list&category_id=' . $id;
    $link   = URL::createLink('frontend', 'book', $action);
